class PuzzleY2021D22 : Puzzle {
  ...
  private data class Step(val on: Boolean, val cuboid: Cuboid)

  private lateinit var steps: List<Step>

  override fun parse(input: String) {
    val regex = Regex("""(on|off) x=(-?\d+\.\.-?\d+),y=(-?\d+\.\.-?\d+),z=(-?\d+\.\.-?\d+)""")
    steps = input.lines().map { line ->
      val match = regex.matchEntire(line)!!.groupValues
      Step(
        match[1] == "on",
        Cuboid(
          match[2].toLongRange(),
  ...
  override fun solve1(): Long {
    val limit = Cuboid(-50L..50L, -50L..50L, -50L..50L)
    return countCuboidsIn(steps.map { Step(it.on, it.cuboid intersect limit) })
  }

  override fun solve2(): Long {
    return countCuboidsIn(steps)
  }
  ...
  companion object {
    ...
    val testInput1_3 = """
      ...
      """.trimIndent()
    val testAnswer1_3 = 474140

    val testInput2 = testInput1_3
    val testAnswer2 = 2758514936282235
  }
}
